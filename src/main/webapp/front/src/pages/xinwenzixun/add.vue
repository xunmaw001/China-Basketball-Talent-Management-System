...
<div :style='{"width":"1200px","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' label="来源" prop="laiyuan">
            <el-input v-model="ruleForm.laiyuan" 
                placeholder="来源" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}'  label="新闻分类" prop="xinwenfenlei">
            <el-select v-model="ruleForm.xinwenfenlei" placeholder="请选择新闻分类" >
              <el-option
                  v-for="(item,index) in xinwenfenleiOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' label="封面背景" prop="fengmianbeijing">
            <file-upload
            tip="点击上传封面背景"
            action="file/upload"
            :limit="3"
            :multiple="true"
            :fileUrls="ruleForm.fengmianbeijing?ruleForm.fengmianbeijing:''"
            @change="fengmianbeijingUploadChange"
            ></file-upload>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' label="编辑人" prop="bianjiren">
            <el-input v-model="ruleForm.bianjiren" 
                placeholder="编辑人" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' label="新闻内容" prop="xinwenneirong">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='xinwenfenlei'){
              this.ruleForm.xinwenfenlei = obj[o];
              continue;
            }
            if(o=='fengmianbeijing'){
              this.ruleForm.fengmianbeijing = obj[o];
              continue;
            }
            if(o=='bianjiren'){
        ...
        this.$http.get('option/xinwenfenlei/xinwenfenlei', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.xinwenfenleiOptions = res.data.data;
          }
        });
      },
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('xinwenzixun/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('xinwenzixun/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('xinwenzixun/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      fengmianbeijingUploadChange(fileUrls) {
          this.ruleForm.fengmianbeijing = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
